...
config = process_config('experiments/2022-04-13/GRU_Chem/config.json')
...
finetuner.finetune()
finetuner.model.save_weights('experiments/2022-04-13/GRU_Chem/checkpoints/finetuned_gen1.hdf5')


elapsed1 = (time.clock() - start)
print("Time used train:",elapsed1)


config = process_config('experiments/2022-04-13/GRU_Chem/config.json')

config['model_weight_filename'] = 'experiments/2022-04-13/GRU_Chem/checkpoints/finetuned_gen1.hdf5'
modeler = GRUChem(config, session='generate')
generator = GRUChemGenerator(modeler)
#print(config)
...
print("Time used for  generate:",elapsed)
...
import pandas as pd
training_data = pd.read_csv('./generated_data/retrain.smi', header=None)
...
with open('./generations/gen_2.smi', 'w') as f:
